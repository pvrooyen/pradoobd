...
```
React UI (Chrome)  ──WebSocket──►  Bridge server (Node/TS)  ──TCP 35000──►  ELM327 WiFi  ──►  Prado OBD-II
```
...
- **Generic Mode 01 PIDs**: partial — some return `NO DATA`. The *Supported PID scan* tells you exactly which ones your ECU answers.
...
- **Actuator tests / forced DPF regen / coding**: generally need Techstream-level UDS sessions — out of reach for a stock ELM327. That's exactly the future the `Transport` abstraction leaves room for.
...
pradoobd/
├─ packages/
│  ├─ shared/   @pradoobd/shared — types, WS message contract, PID tables (standard + Toyota seed list), DTC decoding
│  ├─ server/   @pradoobd/server — the bridge: Transport (ELM327-WiFi | Mock), ELM327 protocol, decoders, WS + static host
│  └─ web/      @pradoobd/web    — Vite + React + TS dashboard (mobile-first)
```

Key seams:
- `server/src/transport/Transport.ts` — the interface that future-proofs the tool. `ElmWifiTransport` is today's impl; `MockTransport` simulates a Prado for desk dev.
- `server/src/protocol/ObdSession.ts` — all OBD knowledge (init, scan, DTC, VIN, live, Mode 22 probe).
- `server/src/decoders/` — `standard.ts` (SAE J1979 formulas) and `toyota.ts` (provisional Mode 22 decoders + a raw-bytes fallback for reverse engineering).
- `shared/src/toyota.ts` — Toyota enhanced PID **candidates**. ⚠️ Scalings here are provisional until confirmed at the car.

---

## Requirements

- Node.js ≥ 20 (developed on 23). npm ≥ 10.
...
**Mock mode — develop at your desk, no car needed.** Simulates the Prado (supported PIDs, two DTCs, Mode 22 hits, drifting live values).
...
**Live mode — at the car.** Same, without `MOCK`:
...
> Tip: `$env:LOG_LEVEL="debug"` on the server prints every byte exchanged with the adapter — invaluable for reverse-engineering Toyota PIDs.
...
2. On the laptop, **join the adapter's WiFi network** (SSID is usually `WiFi_OBDII` / `V-LINK` / similar; no internet on it — that's expected).
3. `npm run build; npm start` (or the two dev terminals). Open the UI.
4. **Connection tab** → Connect (defaults to `192.168.0.10:35000`). Confirm adapter id + negotiated protocol appear. Try **Read VIN**.
5. **Live Data tab** → *Scan supported PIDs* → tick the ones you want → *Start live*. Note which standard PIDs your ECU actually serves.
6. **Trouble Codes tab** → Read DTCs. (Record them before clearing — clearing also resets readiness monitors.)
7. **Mode 22 Prober tab** → Probe the default `0100–01FF` range. For each hit, open the **Terminal** and re-request it (`22 01 19`) while you rev/warm the engine; watch the raw bytes move and derive the real scaling.
...
node packages/server/smoke-test.mjs   # in another — exercises the full WS flow, exits 0 on pass
```
...
- **Confirm Toyota PIDs** → promote provisional decoders to confirmed gauges.
- **Session logging / replay** → record raw adapter traffic to `captures/` and replay through `MockTransport` for offline analysis.
- **CSV / chart export** of live logs.
- **New transports**: J2534 pass-thru, DoIP, or a UDS-capable adapter — implement `Transport` and the rest of the app comes along, unlocking actuator tests and coding.
